    ...
    <>
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="relative max-w-[1120px] mx-auto px-6 pt-24 md:pt-28 pb-6">
          ...
          <p className="text-[14px] text-white/70 leading-[1.7] max-w-lg">Live scores, game results, and upcoming matchups across all teams.</p>
        </div>
      </section>
      ...
      <div className="max-w-[1120px] mx-auto px-6 py-6 space-y-6">
        ...
        {liveGames.length > 0 ? (
              ...
              {liveGames.map((g) => (
                <a key={g.id} href={`/teams/${g.teamId}/game/${g.id}`} className="no-underline block border border-[#17FC13]/20 bg-[#17FC13]/[0.02] p-4 hover:border-[#17FC13]/40 transition-colors">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <span className="text-[9px] text-white/60 uppercase">{g.teamName}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <div>
                      <div className="text-[10px] text-white/70 uppercase">{g.opponent}</div>
                      <div className="text-xl font-bold">{g.linescore.awayRuns}</div>
                    </div>
                    <div className="text-[9px] font-bold uppercase text-white/60">{g.linescore.isTopHalf ? "Top" : "Bot"} {g.linescore.currentInning}</div>
          ...
          <div className="border border-[#171717] bg-radial p-6 text-center">
            <p className="text-xs text-white/65 mb-1">No live games right now</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          {recentFinals.length > 0 && (
            <div className="border border-[#171717]">
              <div className="px-4 py-3 border-b border-[#171717]">
                <span className="text-[9px] font-bold uppercase tracking-[0.2em] text-white/55">Recent Results</span>
              </div>
              {recentFinals.map((g) => {
                ...
                return (
                  <a key={g.id} href={`/teams/${g.teamId}/game/${g.id}`} className="no-underline flex items-center justify-between px-4 py-3 border-b border-[#171717] last:border-b-0 hover:bg-white/[0.01] transition-colors">
                    ...
                    <div className="flex items-center gap-2">
                      <span className={`text-xs font-bold ${won ? "text-[#17FC13]" : "text-white/65"}`}>{won ? "W" : "L"}</span>
          ...
          {upcoming.length > 0 && (
            <div className="border border-[#171717]">
              <div className="px-4 py-3 border-b border-[#171717]">
                <span className="text-[9px] font-bold uppercase tracking-[0.2em] text-white/55">Upcoming</span>
              </div>
              {upcoming.map((g) => (
                <div key={g.id} className="flex items-center justify-between px-4 py-3 border-b border-[#171717] last:border-b-0">
                  <div>
                    <div className="text-[10px] text-white/60">{g.date} &middot; {g.time}</div>
                    <div className="text-xs font-bold uppercase mt-0.5">{g.teamName} vs {g.opponent}</div>
                  </div>
        ...
        {allGames.length === 0 && (
          <div className="border border-[#171717] bg-radial p-8 text-center">
            ...
            <p className="text-[12px] text-white/65 mb-6">Games appear here once coaches start scheduling.</p>
        ...
        <div className="border border-[#171717] bg-radial p-4 flex flex-col sm:flex-row items-center justify-between gap-3">
          <p className="text-[11px] text-white/60">Live scores and box scores for all Apex Academy teams.</p>
          <div className="flex items-center gap-2">
