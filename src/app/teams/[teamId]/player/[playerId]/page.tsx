...
export default function PlayerProfilePage() {
  ...
  if (!team || !player) {
    return <div className="max-w-[1120px] mx-auto px-6 pt-32 pb-20 text-center"><p className="text-white/70">Player not found.</p><Button href={`/teams/${teamId}`} variant="secondary" size="small">Back to Roster</Button></div>;
  }
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="relative max-w-[1120px] mx-auto px-6 pt-24 md:pt-28 pb-6">
          <div className="flex items-center gap-2 mb-5 text-[10px] font-medium uppercase tracking-[0.2em]">
            <a href={`/teams/${teamId}`} className="text-white/60 no-underline hover:text-white/80">{team.age} Roster</a>
          ...
          <div className="border border-[#171717] bg-radial p-6 md:p-8">
            ...
            <div className="grid grid-cols-3 sm:grid-cols-6 gap-x-8 gap-y-6 mb-6">
              <div>
                <div className="text-[9px] font-bold uppercase tracking-[0.2em] text-white/65 mb-1.5">Position</div>
                <div className="text-base md:text-lg font-bold">{player.pos}{player.secondaryPos ? ` / ${player.secondaryPos}` : ""}</div>
              </div>
              <div>
                <div className="text-[9px] font-bold uppercase tracking-[0.2em] text-white/65 mb-1.5">Height</div>
                <div className="text-base md:text-lg font-bold">{player.ht}</div>
              </div>
              <div>
                <div className="text-[9px] font-bold uppercase tracking-[0.2em] text-white/65 mb-1.5">Weight</div>
                <div className="text-base md:text-lg font-bold">{player.wt}</div>
              </div>
              <div>
                <div className="text-[9px] font-bold uppercase tracking-[0.2em] text-white/65 mb-1.5">Class</div>
                <div className="text-base md:text-lg font-bold">{player.year}</div>
              </div>
              <div>
                <div className="text-[9px] font-bold uppercase tracking-[0.2em] text-white/65 mb-1.5">Hometown</div>
                <div className="text-base md:text-lg font-bold">{player.hometown}</div>
              </div>
              {player.school && (
                <div>
                  <div className="text-[9px] font-bold uppercase tracking-[0.2em] text-white/65 mb-1.5">School</div>
            ...
            <div className="grid grid-cols-3 sm:grid-cols-6 gap-x-8 gap-y-6 mb-6">
              <div>
                <div className="text-[9px] font-bold uppercase tracking-[0.2em] text-white/65 mb-1.5">B/T</div>
                <div className="text-base md:text-lg font-bold">{player.bats}-{player.throws}</div>
              </div>
              <div>
                <div className="text-[9px] font-bold uppercase tracking-[0.2em] text-white/65 mb-1.5">Team</div>
            ...
            {(player.instagram || player.twitter) && (
              <div className="flex items-center gap-4 pt-4 border-t border-[#171717]">
                {player.instagram && (
                  <a href={`https://instagram.com/${player.instagram.replace("@", "")}`} target="_blank" rel="noopener noreferrer" className="text-white/65 hover:text-[#17FC13] transition-colors no-underline" title="Instagram">
                ...
                {player.twitter && (
                  <a href={`https://twitter.com/${player.twitter.replace("@", "")}`} target="_blank" rel="noopener noreferrer" className="text-white/65 hover:text-[#17FC13] transition-colors no-underline" title="X / Twitter">
      ...
      <div className="max-w-[1120px] mx-auto px-6 py-6">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
          ...
          <div className="lg:col-span-8 space-y-5">
            ...
            {player.bio && (
              <div className="border border-[#171717] p-5">
                <div className="text-[9px] font-bold uppercase tracking-[0.2em] text-white/55 mb-2">About</div>
                <p className="text-[13px] text-white/90 leading-[1.8]">{player.bio}</p>
              </div>
            )}
                ...
                <div className="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-11 gap-1.5">
                  {[
                    ...
                    <div key={s.l} className="border border-[#171717] py-2 px-1 text-center">
                      <div className="text-[7px] font-bold uppercase tracking-wider text-white/55 mb-0.5">{s.l}</div>
            ...
            {gameLog.length > 0 && (
              <div className="border border-[#171717]">
                <div className="px-5 py-3 border-b border-[#171717]">
                  <span className="text-[9px] font-bold uppercase tracking-[0.2em] text-white/55">Game Log</span>
                </div>
                    ...
                    <thead>
                      <tr className="border-b border-[#171717] bg-white/[0.01]">
                        {["Date", "Opp", "", "AB", "H", "HR", "RBI", "BB", "K"].map((h) => (
                          <th key={h} className={`${h === "Date" || h === "Opp" ? "text-left px-4" : "text-center px-2"} py-2 font-bold uppercase tracking-wider text-white/60`}>{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                      {gameLog.map((g) => (
                        <tr key={g.gameId} className="border-b border-[#171717] last:border-b-0 hover:bg-white/[0.01]">
                          <td className="px-4 py-2 text-white/70">{g.date}</td>
                          <td className="px-4 py-2 font-bold uppercase text-white/90">
                            <a href={`/teams/${teamId}/game/${g.gameId}`} className="text-white/90 no-underline hover:text-[#17FC13]">{g.opponent}</a>
                          </td>
                          <td className="px-2 py-2 text-center"><span className={`font-bold ${g.result.startsWith("W") ? "text-[#17FC13]" : "text-white/65"}`}>{g.result.split(" ")[0]}</span></td>
                          <td className="px-2 py-2 text-center text-white/80">{g.AB}</td>
                          <td className="px-2 py-2 text-center text-white/80">{g.H}</td>
                          <td className="px-2 py-2 text-center text-white/80">{g.HR}</td>
                          <td className="px-2 py-2 text-center text-white/80">{g.RBI}</td>
                          <td className="px-2 py-2 text-center text-white/80">{g.BB}</td>
                          <td className="px-2 py-2 text-center text-white/80">{g.K}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
            ...
            {player.achievements && player.achievements.length > 0 && (
              <div className="border border-[#171717] p-5">
                <div className="text-[9px] font-bold uppercase tracking-[0.2em] text-white/55 mb-3">Achievements</div>
                <div className="flex flex-wrap gap-1.5">
                  {player.achievements.map((a, i) => (
                    <span key={i} className="inline-flex items-center gap-1.5 px-2.5 py-1 border border-[#171717] text-[10px] font-bold uppercase tracking-wider text-white/80">
          ...
          <div className="lg:col-span-4 space-y-5">
            ...
            {/* Teammates */}
            <div className="border border-[#171717] p-5">
              <div className="text-[9px] font-bold uppercase tracking-[0.2em] text-white/55 mb-3">Teammates</div>
                ...
                {teammates.map((t) => (
                  <a key={t.number} href={`/teams/${teamId}/player/${t.number}`} className="flex items-center justify-between py-1.5 no-underline group">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[11px] font-bold uppercase text-white/85 group-hover:text-[#17FC13] transition-colors">{t.name}</span>
                    </div>
                    <span className="text-[9px] text-white/55">{t.pos}</span>
                  </a>
                ))}
              </div>
              <a href={`/teams/${teamId}`} className="block mt-3 pt-3 border-t border-[#171717] text-[10px] font-bold uppercase tracking-wider text-white/55 no-underline hover:text-[#17FC13] transition-colors text-center">
                Full Roster
              </a>
            </div>
          </div>
