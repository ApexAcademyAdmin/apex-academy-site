...
export function PlayFeed({ atBats }: { atBats: AtBat[] }) {
  ...
  if (sorted.length === 0) {
    return <p className="text-sm text-white/65">No plays recorded yet.</p>;
  }
    ...
    <div className="space-y-0 border border-[#171717]">
      {sorted.map((ab) => (
        <div key={ab.id} className="px-4 py-3 border-b border-[#171717] last:border-b-0 flex items-start gap-3">
          <span className="text-[9px] font-bold uppercase tracking-wider text-white/60 shrink-0 mt-0.5 w-14">
